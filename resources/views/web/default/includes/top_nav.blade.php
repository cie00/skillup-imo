                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu user-profile-dropdown text-white" aria-labelledby="dropdownMenuButton">
                        ...
                        <a class="dropdown-item" href="{{ $authUser->isAdmin() ? '/admin' : '/panel' }}">
                            ...
                            <span class="font-14 text-color-blue">{{ trans('public.my_panel') }}</span>
                        </a>
                        @if($authUser->isTeacher() or $authUser->isOrganization())
                            <a class=" text-whitedropdown-item" href="{{ $authUser->getProfileUrl() }}">
                                <img src="/assets/default/img/icons/profile.svg" width="25" alt="nav-icon">
                                <span class="font-14 text-color-blue">{{ trans('public.my_profile') }}</span>
                            </a>
                        @endif
                        <a class="dropdown-item" href="/logout">
                            <img src="/assets/default/img/icons/sidebar/logout.svg" width="25" alt="nav-icon">
                            <span class="font-14 text-color-blue">{{ trans('panel.log_out') }}</span>
                        </a>
                    </div>
                </div>
            @else
                <div class="d-flex align-items-center ml-md-50">
                    <a href="/login" class="py-5 px-10 mr-10 text-white font-14">{{ trans('auth.login') }}</a>
                    <a href="/register" class="py-5 px-10 text-white font-14">{{ trans('auth.register') }}</a>
                </div>
            @endif
        </div>
    </div>
